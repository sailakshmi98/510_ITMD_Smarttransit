...
public class BusController extends Thread implements Initializable {
	  ...
	  @FXML
	    ...
	    @FXML
	    private TableColumn<ServiceModel,String>service1;
	    @FXML
	    private TableColumn<ServiceModel,String>source1;
	    @FXML
	    private TableColumn<ServiceModel,String>destination1;
	    @FXML
	    private TableColumn<ServiceModel,Integer>fare1;
	    @FXML
	    private TableColumn<ServiceModel,Time> depart1;
	    @FXML
	    private TableColumn<ServiceModel, Time>arrival1;
	    @FXML
	    private TableColumn<ServiceModel,Integer>seat1;
	    @FXML
	    private TableColumn<ServiceModel,String>date1;
  ...
  @FXML
  private TableView<ServiceModel> tableview;
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
	    ...
	    source1.setCellValueFactory(new PropertyValueFactory<>("source"));
	    destination1.setCellValueFactory(new PropertyValueFactory<>("destination"));
	    fare1.setCellValueFactory(new PropertyValueFactory<>("fare"));
	    depart1.setCellValueFactory(new PropertyValueFactory<>("depart"));
	    arrival1.setCellValueFactory(new PropertyValueFactory<>("arrival"));
	    seat1.setCellValueFactory(new PropertyValueFactory<>("seat"));
  ...
  public void connect() {
	    ...
	    try {
	        ...
	        while (resultSet.next()) {
	            String fromValue = resultSet.getString("from");
	            String toValue = resultSet.getString("to");

	            if (fromValue != null) {
	                from.getItems().addAll(fromValue);
	            }
	            if (toValue != null) {
	                to.getItems().addAll(toValue);
	            }
	        }
  ...
  private void loaddata() {
      ...
      ConnectionClass connectionClass_search = new ConnectionClass();
      Connection connection_search = connectionClass_search.getConnection();
      
      try {
      	PreparedStatement search = connection_search.prepareStatement(
      		    "CREATE TABLE IF NOT EXISTS smart_transit_search (" +
	    ...
	    if (from.getSelectionModel().getSelectedItem() != null && to.getSelectionModel().getSelectedItem() != null ) {
	        ...
	        try {
	            pst = connection.prepareStatement(
	                "SELECT * FROM smart_transit_search WHERE source = ? AND dest = ?");
	        ...
	        // Show an alert or message to let the user know that they need to select all the fields
	    }
	}
  ...
  public void bookticket(ActionEvent event) throws IOException {
    ...
    book.setOnAction(e -> {
      ...
      if ((list6.get(0) == 1 && bta1)
        ...
        ServiceModel selectedrow = tableview.getSelectionModel().getSelectedItem();
        if (selectedrow == null) {
          Alert alert = new Alert(Alert.AlertType.ERROR);
  ...
  private void onRowClicked(MouseEvent event) {
	    if (event.getClickCount() == 2) { // Double-click event
	        try {
	            ServiceModel selectedService = table.getSelectionModel().getSelectedItem();
	            if (selectedService != null) {
	                FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/BusDetails.fxml"));
	                Parent parent = loader.load();

	                BusDetailsController busDetailsController = loader.getController();
	                busDetailsController.initData(selectedService);
